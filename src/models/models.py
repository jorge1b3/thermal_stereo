...
class ThermalDepthModelResNet(nn.Module):
    ...
    def __init__(self, initial_filters=64, dropout_rate=0.2):
        ...
        # Codificador para imagen térmica izquierda
        self.left_conv1 = nn.Conv2d(3, initial_filters, kernel_size=7, stride=2, padding=3)
        self.left_bn1 = nn.BatchNorm2d(initial_filters)
        self.left_res1 = self._make_residual_block(initial_filters, initial_filters*2)
        self.left_res2 = self._make_residual_block(initial_filters*2, initial_filters*4)
        self.left_res3 = self._make_residual_block(initial_filters*4, initial_filters*4)
        self.left_dropout = nn.Dropout(dropout_rate)
        
        # Codificador para imagen térmica derecha
        self.right_conv1 = nn.Conv2d(3, initial_filters, kernel_size=7, stride=2, padding=3)
        self.right_bn1 = nn.BatchNorm2d(initial_filters)
        self.right_res1 = self._make_residual_block(initial_filters, initial_filters*2)
        self.right_res2 = self._make_residual_block(initial_filters*2, initial_filters*4)
        self.right_res3 = self._make_residual_block(initial_filters*4, initial_filters*4)
        self.right_dropout = nn.Dropout(dropout_rate)
        
        # Decodificador con bloques residuales
        self.up_conv1 = nn.ConvTranspose2d(initial_filters*8, initial_filters*4, kernel_size=4, stride=2, padding=1)
        self.up_bn1 = nn.BatchNorm2d(initial_filters*4)
        
        self.up_conv2 = nn.ConvTranspose2d(initial_filters*4, initial_filters*2, kernel_size=4, stride=2, padding=1)
        self.up_bn2 = nn.BatchNorm2d(initial_filters*2)
        
        self.up_conv3 = nn.ConvTranspose2d(initial_filters*2, initial_filters, kernel_size=4, stride=2, padding=1)
...
class ThermalDepthModelUNet(nn.Module):
    ...
    def __init__(self, initial_filters=64, dropout_rate=0.2):
        ...
        self.left_enc3 = self._make_encoder_block(initial_filters*2, initial_filters*4)
        self.left_enc4 = self._make_encoder_block(initial_filters*4, initial_filters*8)
        ...
        self.right_enc3 = self._make_encoder_block(initial_filters*2, initial_filters*4)
        self.right_enc4 = self._make_encoder_block(initial_filters*4, initial_filters*8)
...
def get_model(model_name='basic', **kwargs):
    ...
    if model_name not in MODEL_REGISTRY:
        raise ValueError(f"Modelo '{model_name}' no encontrado. Opciones disponibles: {list(MODEL_REGISTRY.keys())}")
