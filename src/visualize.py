...
def visualize_predictions(
    model, test_loader, device, save_dir="results", num_samples=5
):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualiza las predicciones del modelo"
    )
    parser.add_argument(
        "--model_path", type=str, required=True, help="Ruta al modelo entrenado"
    )
    parser.add_argument(
        "--data_dir",
        type=str,
        default="raw",
        help="Directorio raíz donde se encuentran los datos",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=1,
        help="Tamaño del lote (debe ser 1 para visualización)",
    )
    parser.add_argument(
        "--model_type",
        type=str,
        default="basic",
        choices=["basic", "resnet", "unet"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=64,
        help="Número inicial de filtros del modelo",
    )
    parser.add_argument(
        "--dropout_rate", type=float, default=0.2, help="Tasa de dropout del modelo"
    )
    parser.add_argument(
        "--save_dir",
        type=str,
    ...
    parser.add_argument(
        "--num_samples", type=int, default=5, help="Número de muestras a visualizar"
    )
    ...
    test_loader = load_test_dataset(
        root_dir=args.data_dir, batch_size=args.batch_size, num_workers=1
    )
