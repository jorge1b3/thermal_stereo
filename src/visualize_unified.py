...
def parse_args():
    parser = argparse.ArgumentParser(description="Visualizar resultados del modelo unificado")
    parser.add_argument('--model_path', type=str, required=True, help='Ruta al modelo guardado')
    parser.add_argument('--data_dir', type=str, default='raw', help='Directorio de datos')
    parser.add_argument('--split', type=str, default='test', help='Split a visualizar (test, val)')
    parser.add_argument('--initial_filters', type=int, default=32, help='Número de filtros iniciales del modelo')
    parser.add_argument('--max_disp', type=int, default=192, help='Disparidad máxima del modelo')
    parser.add_argument('--output_dir', type=str, default='outputs', help='Directorio para guardar visualizaciones')
    parser.add_argument('--num_samples', type=int, default=10, help='Número de muestras a visualizar')
...
def visualize_predictions(model, dataloader, device, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            right_np = (right_np - right_np.min()) / (right_np.max() - right_np.min() + 1e-8)
            
            # Normalizar mapas de profundidad para visualización
            gt_depth_norm = (gt_depth_np - gt_depth_np.min()) / (gt_depth_np.max() - gt_depth_np.min() + 1e-8)
            pred_stereo_norm = (pred_stereo_np - pred_stereo_np.min()) / (pred_stereo_np.max() - pred_stereo_np.min() + 1e-8)
            pred_mono_norm = (pred_mono_np - pred_mono_np.min()) / (pred_mono_np.max() - pred_mono_np.min() + 1e-8)
